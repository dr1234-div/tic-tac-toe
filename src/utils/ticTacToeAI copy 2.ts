...
export class GameState {
    ...
    getScore = memoize(() => {
        ...
        for (let index = 0; index < availablePos.length; index++) {
            ...
            // 在给定的位置下子，生成一个新的棋盘
            const newChessArr = this.generateNewBoard({
                row,
                col,
                chess: this.player,
            });
    ...
    initializeChessScore = (chessWidth:number) => {
        const chessScore = Array(chessWidth)
            .fill([])
            .map(() => Array(chessWidth).fill('无棋子'));
    ...
    computerDown = () => {
        ...
        const score = Array(chessWidth)
            .fill([])
            .map(() => Array(chessWidth).fill(0));
        const chessPlace = this.initializeChessScore(chessWidth);
        ...
        for (let row = 1; row < 15; row++) {
            for (
                let index = row, col = chessWidth - 1;
                col >= 0 && index < 15;
                col--, index++
            ) {
                let tempM = index;
        ...
        for (let row = 0; row < chessWidth - 1; row++) {
            for (
                let index = row, col = 0;
                col < chessWidth && index < chessWidth;
                col++, index++
            ) {
                let tempM = index;
                let tempN = col;
        ...
        for (let row = 1; row < chessWidth - 4; row++) {
            for (
                ...
                for (
                    ;
                    tempM < index + 5 && index + 5 <= chessWidth;
                    tempM++, tempN--
                ) {
                    if (
                        chessPlace[tempM][tempN] === '先手'
    ...
    getWinner = (
        ...
        for (const direction of directions) {
            ...
            if (
                lodash.compact(flattenedArr).length ===
                    this.gameConfig.chessBorder ** 2 &&
                count < this.gameConfig.winCount
            ) {
                return '平局';
            }
            if (
